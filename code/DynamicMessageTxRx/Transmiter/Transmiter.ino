...
#define AUTO_MSG false
#define USER_MSG true
...
void loop()
{
    ...
    {   
        if (Serial.available())
        {
            ...
            const char* userInputMsg = userInput.c_str();
            
            if (strlen(userInputMsg) <= 16)
            {
                /* Setup for LCD */
                LCD_Init(userInputMsg, LCD_LINES, LCD_ROWS);

                /* Send data */
                SSCC_SendData(userInputMsg, TX_RATE, TX_DATA, TX_CLOCK);
            }
            else
            {
                if (Serial.availableForWrite())
                {
                    Serial.println("Message longer than 16 characters !");
                }
            }
